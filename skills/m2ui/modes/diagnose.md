...
> **EXTREMELY-IMPORTANT — when fixing a "Missing `@ui.WindowDestroy`" finding specifically:** apply Critical Rule 17 (SKILL.md) in full — ADD the decorator, never strip the existing body; guard direct owned-widget calls with `if self.X:`; inspect every helper the body calls and guard widget derefs inside it too. Stripping the body deletes user intent and is a regression. Other findings in the same audit are independent — fix each per its own rule; this warning constrains only the decorator-add. Details: `reference/patterns.md` Section 5.11.
