...
6. **`GetChild` raises `KeyError` during `__LoadWindow`** — a `GetChild("X")` with no matching child `"name" : "X"` in the uiscript aborts the whole open flow, often surfacing as a misleading error higher up the call chain (e.g. a failed server-command log). Classic case: asking for the root window dict's own `name` — the loader registers only its `children`; the window itself is `self`. `GetChild` raises, it never returns `None`, so `GetChild(x) or fallback` masks nothing. Fix: cross-check every `GetChild` against the registered names — uiscript children tree plus any `InsertChild` calls (Critical Rule 20).
...
6. **Same asset base loaded with different extensions at different sites** — one call site loads `base + ".sub"`, another `base + ".tga"`; at most one exists, so one widget renders nothing (or the red X). Fix: confirm the real on-disk extension once, use it at every call site.
...
2. **`OnMouseWheel` missing `return True`** — Scrollbar hover area handles the wheel but returns None, so the engine bubbles the wheel to the parent (which scrolls something else, or nothing). Fix: `def OnMouseWheel(self, length): self.scrollbar.OnUp() if length > 0 else self.scrollbar.OnDown(); return True`. Placement: put the handler on the WINDOW, not per list button — `RunMouseWheel` bubbles up the parent chain, so wheel over an unhandled child reaches the window; a sibling that consumes its own wheel (returns `True`) keeps working independently.
...
6. **Scrolls only one direction / thumb stuck (sub-pixel wheel steps)** — base `ScrollBar.SetPos` floors with `int(newPos)`. With an enlarged thumb, `pageSize` is small and a wheel step can be sub-pixel: down `int(px + frac) = px` (stuck) while up `int(px - frac) = px - 1` (moves) — and `OnMove` writes the quantized pixel back into the position, locking it. Fix: augment `SetPos` to `int(round(newPos))` (symmetric; drag unaffected) — see `skills/m2ui/reference/framework-augmentations.md` ScrollBar section.
...
4. **`int(color, 16)` applied to an int** — color table stores packed int literals (`0xFFAABBCC`) but a call site treats them as hex strings: `SetPackedFontColor(int(color, 16) | 0xFF000000)` raises `TypeError` in Python 2 (crashing whatever handler runs it, e.g. on every appear event). Fix: store packed ARGB ints with alpha baked in and pass them straight to `SetPackedFontColor(color)`; keep ONE representation across every site that reads the table.

**Quick check:** Set the color literal to `0xFFFF0000` (opaque red in engine `0xAARRGGBB`). Renders red → channel order is fine, issue is elsewhere. Renders blue/transparent → the code path treats literals as another order (web `0xRRGGBBAA` or `0xAABBGGRR`) — root cause #1 or #2.
