...
- `QuestionDialog2` — two message lines (uses `questiondialog2.py`). Exposes `SetText1()` / `SetText2()` for per-line assignment, and `SetText(text)` which auto-splits on `\n` and delegates to `SetText1` / `SetText2`. Prefer `SetText()` when a single locale key contains the full two-line message; use `SetText1()` / `SetText2()` when lines come from separate keys or are set conditionally. `SetText()` requires augmented `uiCommon.py` — see `reference/framework-augmentations.md`.
